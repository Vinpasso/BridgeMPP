...
public class Message
{
	...
	{
		return messageFormat.convertToClosestFormat(message, formats).getValue();
	}

	public MessageFormat chooseMessageFormat(MessageFormat... formats)
	{
		return messageFormat.convertToClosestFormat(message, formats).getKey();
	}

	public Entry<MessageFormat, String> getClosestConversion(MessageFormat... messageFormats)
	{
		return messageFormat.convertToClosestFormat(message, messageFormats);
	}
	...
	/**
	 * Return the End-User friendly Description of this Message This attempts to
	 * format the Message in the following way: <sender>: Message If the sender
	 * is unavailable the origin is used instead If the origin is unavailable
	 * unknown will be used
	 * 
	 * @param messageFormats
	 *            The Messages Format to return the message in
	...
	/**
	 * This returns a more detailed description of the Message in the following
	 * format <Message Format>: <Group>: <Sender> (<Origin>) --> <Destination>:
	 * <Message Format>" This is the default format in the toString Method
	 * (Plain-Text)
	 * 
	 * @param messageFormats
	 *            The requested format to return the message in
	 * @return The formatted Message
	 */
