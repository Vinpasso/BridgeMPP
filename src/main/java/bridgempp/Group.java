...
public class Group {
		...
		for (int i = 0; i < getEndpoints().size(); i++) {
			try {
				...
				ShadowManager.log(Level.WARNING,
						"Delivery failed! Message could not be delivered to "
								+ getEndpoints().get(i).toString(true), e);
				message.getSender().sendOperatorMessage(
						"Delivery failed! Message could not be delivered to "
								+ getEndpoints().get(i).toString(true));
			}		}
	}
	...
	public String toString() {
		String members = "";
		for (int i = 0; i < getEndpoints().size(); i++) {
			members += "Member: " + getEndpoints().get(i).toString(false)
					+ "Alias: " + getEndpoints().get(i).toString(true) + "\n";
		}
		return members;
	}
